...
const getProducts = async (req, res) => {
  try {
    ...
    if (page <= 0) {
      return res
        .status(400)
        .json(new ApiError(400, "page should be gretaer than 0"));
    }
    ...
    if (products.length == 0) {
      return res
        .status(404)
        .json(new ApiError(400, "You have reached the end of the list."));
    }

    return res.status(200).json(new ApiResponse(200, products, "Data Fetched"));
  } catch (error) {
    console.log(error);
    return res.status(500).json(new ApiError(500, "Internal Server Error"));
  }
